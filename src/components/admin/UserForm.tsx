...
const userFormObject = z.object({
  ...
  role: z.enum(['customer', 'seller', 'admin', 'courier', 'supervisor'], {
...
export function UserForm({ user, currentUserRole, onSubmit, onCancel }: UserFormProps) {
  ...
  const [availableBusinessTowns, setAvailableBusinessTowns] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    if (
      (watchedRole === 'seller' || watchedRole === 'courier') &&
      watchedBusinessRegion &&
      watchedBusinessRegion !== NO_REGION_VALUE
    ) {
      setAvailableBusinessTowns(
        GHANA_REGIONS_AND_TOWNS[watchedBusinessRegion] || []
      );
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    if (!result.success || !result.url) {
      throw new Error(
        result.message || `API error when uploading ${file.name}.`
      );
    }
  ...
  const handleSubmit = async (values: UserFormValues) => {
    const dataToSubmit: Partial<User> = {
      ...
      town:
        values.town === NO_TOWN_VALUE || !values.town ? undefined : values.town,
      businessLocationRegion:
        values.businessLocationRegion === NO_REGION_VALUE
          ? undefined
          : values.businessLocationRegion,
      businessLocationTown:
        values.businessLocationTown === NO_TOWN_VALUE || !values.town
          ? undefined
          : values.town,
    };

    const fileFields: (keyof UserFormValues)[] = [
      'tradeLicenseFile',
      'nationalIdFile',
      'policeClearanceFile',
      'driverLicenseFile',
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g., 123 Farm Road, Agri-Town"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g., Sole Proprietorship, Cooperative"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      onValueChange={value =>
                        field.onChange(
                          value === NO_REGION_VALUE ? undefined : value
                        )
                      }
                      value={field.value ?? NO_REGION_VALUE}
                    >
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      onValueChange={value =>
                        field.onChange(
                          value === NO_TOWN_VALUE ? undefined : value
                        )
                      }
                      value={field.value ?? NO_TOWN_VALUE}
                      disabled={
                        !watchedBusinessRegion ||
                        watchedBusinessRegion === NO_REGION_VALUE ||
                        availableBusinessTowns.length === 0
                      }
                    >
                      <FormControl>
                        <SelectTrigger>
                      ...
                      <SelectContent>
                        <SelectItem value={NO_TOWN_VALUE}>
                          Select a town (Optional)
                        </SelectItem>
          ...
          <>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {user?.policeClearanceUrl && (
                    <FormDescription>
                      {' '}
                      Current file:{' '}
                      <a
                        href={user.policeClearanceUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-primary underline"
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>License Category/Type</FormLabel>
                    <FormControl>
                      <Input {...field} />
                    </FormControl>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g. Motorcycle, Van" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {user?.vehicleInsuranceUrl && (
                      <FormDescription>
                        {' '}
                        Current file:{' '}
                        <a
                          href={user.vehicleInsuranceUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-primary underline"
                        >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {user?.roadworthinessUrl && (
                      <FormDescription>
                        {' '}
                        Current file:{' '}
                        <a
                          href={user.roadworthinessUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-primary underline"
                        >
