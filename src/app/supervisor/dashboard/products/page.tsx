...
export default function SupervisorProductsPage() {
  ...
  const handleProductFormSubmit = async (productData: Product) => {
    if (editingProduct) {
      ...
      toast({ title: "Product Updated", description: `Product "${productData.name}" details saved.` });
    } else {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { id, ...newProductData } = productData;
      await addProduct(newProductData);
      toast({ title: "Product Added", description: `New product "${productData.name}" listed.` });
    }
