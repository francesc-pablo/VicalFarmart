...
import { SUPERVISOR_DASHBOARD_NAV_ITEMS } from "@/lib/constants";
...
export default function SupervisorDashboardLayout({
  ...
  if (isLoading || !supervisorUser) {
    return <div className="flex h-screen items-center justify-center">Loading Supervisor Dashboard...</div>;
  }
      ...
      <div className="flex min-h-screen">
        <Sidebar className="border-r bg-sidebar text-sidebar-foreground" collapsible="icon">
          ...
          <SidebarContent className="p-2">
            <SidebarNav items={SUPERVISOR_DASHBOARD_NAV_ITEMS} />
          </SidebarContent>
          <SidebarFooter className="p-4 border-t border-sidebar-border">
             <div className="flex items-center gap-2 group-data-[collapsible=icon]:justify-center">
              <Avatar className="h-8 w-8">
                <AvatarImage src={supervisorUser.avatarUrl || "https://placehold.co/40x40.png"} alt="Supervisor Avatar" data-ai-hint="person face formal" />
                <AvatarFallback>{getInitials(supervisorUser.name)}</AvatarFallback>
              </Avatar>
              <div className="group-data-[collapsible=icon]:hidden">
                <p className="text-sm font-medium">{supervisorUser.name}</p>
                <p className="text-xs text-sidebar-foreground/70">{supervisorUser.email}</p>
              </div>
            </div>
