...
import { COURIER_DASHBOARD_NAV_ITEMS } from "@/lib/constants";
...
export default function CourierDashboardLayout({
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (docSnap.exists() && docSnap.data().role === 'courier') {
          setCourierUser({ id: docSnap.id, ...docSnap.data() } as User);
        } else {
          toast({ title: "Access Denied", description: "You must be a registered courier to view this page.", variant: "destructive" });
          router.push("/login");
        }
      } else {
        router.push("/login");
  ...
  const getInitials = (name?: string) => (name || "C").split(" ").map(n => n[0]).join("").substring(0,2).toUpperCase();

  if (isLoading || !courierUser) {
    return <div className="flex h-screen items-center justify-center">Loading Courier Dashboard...</div>;
  }
      ...
      <div className="flex min-h-screen">
        <Sidebar className="border-r bg-sidebar text-sidebar-foreground" collapsible="icon">
          ...
          <SidebarContent className="p-2">
            <SidebarNav items={COURIER_DASHBOARD_NAV_ITEMS} />
          </SidebarContent>
          <SidebarFooter className="p-4 border-t border-sidebar-border">
             <div className="flex items-center gap-2 group-data-[collapsible=icon]:justify-center">
              <Avatar className="h-8 w-8">
                <AvatarImage src={courierUser.avatarUrl || "https://placehold.co/40x40.png"} alt="Courier Avatar" data-ai-hint="person face" />
                <AvatarFallback>{getInitials(courierUser.name)}</AvatarFallback>
              </Avatar>
              <div className="group-data-[collapsible=icon]:hidden">
                <p className="text-sm font-medium">{courierUser.name}</p>
                <p className="text-xs text-sidebar-foreground/70">{courierUser.email}</p>
              </div>
            </div>
